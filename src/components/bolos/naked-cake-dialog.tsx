...
const tipos = [
  { id: "simples", nome: "Simples", preco: 120 },
  { id: "recheado", nome: "Recheado", preco: 150 },
]
...
interface NakedCakeDialogProps {
  isOpen: boolean
  onClose: () => void
  onAddToCart: (produto: Produto) => void
}

export function NakedCakeDialog({ isOpen, onClose, onAddToCart }: NakedCakeDialogProps) {
  ...
  const handleAddToCart = () => {
    ...
    const produto: Produto = {
      id: `naked-cake-${Date.now()}`,
      nome: `Naked Cake ${tipoSelecionado.nome}`,
      descricao: "",
      tipo: tipoSelecionado.nome,
      preco: tipoSelecionado.preco,
      dataEntrega: dataEntrega ? format(dataEntrega, "dd/MM/yyyy") : "",
      imagens: [{ src: "/naked-cake.jpg", alt: "Naked Cake", description: `Naked Cake ${tipoSelecionado.nome}` }],
    }
    onAddToCart(produto)
    onClose()
    resetForm()
